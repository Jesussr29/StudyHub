...
import { Ban, CheckCircle, Eye, EyeOff, Pencil, Trash2 } from 'lucide-react';
...
export default function AdminIndex({ user, students, teachers, courses }: Props) {
  ...
  const renderActions = (item: any, type: string) => (
    <div className="flex space-x-2">
      ...
      {type === 'students' || type === 'teachers' ? (
        <Button
          variant={item.isBanned ? 'secondary' : 'outline'}
          size="icon"
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-[90%] p-6 relative">
        {/* Mensaje flotante */}
        {message && (
          <div className="fixed top-4 right-4 z-50">
            <div className="rounded-xl bg-green-100 border border-green-400 text-green-800 px-4 py-2 shadow-lg text-sm max-w-sm">
              {message}
            </div>
          </div>
        )}
